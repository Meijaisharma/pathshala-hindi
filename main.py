...
# --- 1. HOME PAGE (INK WASH + CUSTOM BOTTOM NAV) ---
@routes.get('/')
async def index_page(request):
    ...
    <head>
        ...
        <style>
            :root {
                ...
                --text: #4A4A4A; /* Ink Wash Dark Grey */
                --slate: #6D8196; /* Ink Wash Slate */
                --yellow: #FFD600; /* Ink Wash Yellow */
                --card-bg: #FFFFFF;
            }
            * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
            ...
            .header {
                ...
                background: rgba(255, 255, 227, 0.95); backdrop-filter: blur(12px);
            ...
            .thumb {
                width: 100px; height: 75px; background: var(--text); border-radius: 12px;
            ...
            .play-btn {
                position: absolute; width: 32px; height: 32px; background: var(--yellow);
            ...
            /* EXACT CUSTOM BOTTOM NAV (PILL SHAPE) */
            .nav-wrapper {
            ...
            .check-btn svg { width: 18px; height: 18px; stroke-width: 3; }
    ...
    <body>
        ...
        <!-- EXACT PILL NAVIGATION -->
        <div class="nav-wrapper">
            <div class="pill-nav">
                <a href="/?tab=lectures" class="home-btn {HOME_ACT}">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
                </a>
                <a href="/?tab=notes" class="check-btn">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"><polyline points="20 6 9 17 4 12"></polyline></svg>
                    NOTES
                </a>
            </div>
        </div>
...
# --- 2. PLAYER PAGE (VIDSTACK - VLC/PLAYIT FEATURES) ---
@routes.get('/player')
async def player_page(request):
    ...
    <head>
        ...
        <title>Watching Class</title>
        <!-- VIDSTACK PLAYER (Pro Features) -->
        ...
        <style>
            ...
            /* HIDE DOWNLOAD BUTTON IN PLAYER */
            ...
            .back-btn {
                ...
                background: var(--text); color: white; padding: 12px;
    ...
    <body>
        <div class="player-container">
            <media-player title="Class {CID}" src="/stream/{MID}" aspect-ratio="16/9" autoplay controlslist="nodownload">
                ...
                <!-- FULL LAYOUT: Gestures, Speed, PIP, Quality -->
...
async def view_pdf(request):
    ...
    <head>
        <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
        <style>body{margin:0; height:100vh; display:flex; flex-direction:column; background:#222;}</style>
    </head>
    <body>
        <div style="height:50px; background:#111; display:flex; align-items:center; justify-content:space-between; padding:0 20px; color:white; font-family:sans-serif;">
            <span style="font-weight:bold; font-size:14px;">Secure Viewer</span>
...
# --- 4. SECURE STREAM ENGINE (NO DOWNLOAD) ---
...
async def media_handler(request):
    try:
        ...
        if 'video' in mime: mime = 'video/mp4'
        elif 'pdf' in mime: mime = 'application/pdf'
        ...
        headers = {
            ...
            'Content-Disposition': 'inline', # <--- NO DOWNLOAD, ONLY PLAY
            'Accept-Ranges': 'bytes',
        }
...
async def main():
    ...
    await site.start()
    print(f"🚀 INK WASH PRO SERVER: {port}")
